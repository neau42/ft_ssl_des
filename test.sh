...
echo "\n\033[92m - - - - - - - - TEST DES-CFB ENCODE/DECODE with key - vector - - - - -\033[0m"
for i in {1..$1}
do
	str="`cat /dev/urandom | base64 -b $i | head -n 1`"
	echo -n $str | ./ft_ssl des-cfb -a -v 4242123 -k FFAA42 -o test.txt
	decode="`./ft_ssl des-cfb -a -v 4242123 -k FFAA42 -d -i test.txt`"
	if [ "`echo $decode`" = "`echo $str`" ] ; then
		printf "no diff with str (len: %d) encode DES\r" "`echo -n \"$str\" | wc -c`"
	else
		printf "\033[31mERROR\033[0m diff with '%s' (len: %d)\nwanted: [%s]\nfound : [%s]\n" "$str" "`echo -n \"$str\" | wc -c`" "$str" "$decode"
	fi
...
echo "\n\033[92m - - - - - - - - TEST DES-ctr ENCODE/DECODE with key - vector - - - - -\033[0m"
for i in {1..$1}
do
	str="`cat /dev/urandom | base64 -b $i | head -n 1`"
	echo -n $str | ./ft_ssl des-ctr -a -v 4242123 -k FFAA42 -o test.txt
	decode="`./ft_ssl des-ctr -a -v 4242123 -k FFAA42 -d -i test.txt`"
	if [ "`echo $decode`" = "`echo $str`" ] ; then
		printf "no diff with str (len: %d) encode DES\r" "`echo -n \"$str\" | wc -c`"
	else
		printf "\033[31mERROR\033[0m diff with '%s' (len: %d)\nwanted: [%s]\nfound : [%s](len : %d)\n" "$str" "`echo -n \"$str\" | wc -c`" "$str" "$decode" "`echo -n \"$decode\" | wc -c`"
	fi
done
echo ""

echo "\n\033[92m - - - - - - - - TEST DES-pcbc ENCODE/DECODE with key - vector - - - - -\033[0m"
for i in {1..$1}
do
	str="`cat /dev/urandom | base64 -b $i | head -n 1`"
	echo -n $str | ./ft_ssl des-pcbc -a -v 42123 -k FFAA42 -o test.txt
	decode="`./ft_ssl des-pcbc -a -v 42123 -k FFAA42 -d -i test.txt`"
	if [ "`echo $decode`" = "`echo $str`" ] ; then
		printf "no diff with str (len: %d) encode DES\r" "`echo -n \"$str\" | wc -c`"
	else
		printf "\033[31mERROR\033[0m diff with '%s' (len: %d)\nwanted: [%s]\nfound : [%s]\n" "$str" "`echo -n \"$str\" | wc -c`" "$str" "$decode"
