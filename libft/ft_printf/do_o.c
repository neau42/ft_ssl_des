...
/*   Updated: 2016/11/19 02:39:04 by nboulaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static long double	chk_o_modifier(va_list *pa, t_flags *flag, char type)
{
	long double nb;

	if (type == 'O' || flag->f_char & F_C_L)
		...
		nb = (unsigned int)va_arg(*pa, int);
	return (nb);
}

static void			do_o_f_l_moins(int *t, char *buf, t_buf *b)
{
	if (t[2] && t[0]--)
		add_in_buf(b, ' ');
	if (b->flag->f_precision > 0)
		...
		while (t[0]-- > t[3])
			add_in_buf(b, '0');
	else if (buf[0] != '0' && b->flag->f_length & F_L_DIEZE && (t[0]-- || 1))
		add_in_buf(b, '0');
	t[3] = -1;
	while (buf[++t[3]] && (t[0]-- || 1))
		add_in_buf(b, buf[t[3]]);
	while (t[0]-- > (b->flag->f_length & F_L_DIEZE))
		add_in_buf(b, ' ');
}

static void			do_o_wthout_f_l_moins(int *t, char *buf, t_buf *b)
{
	if (!(b->flag->f_length & F_L_ZERO))
		while (t[0] > b->flag->f_precision + t[2] +
			(b->flag->f_length & F_L_DIEZE) &&
			t[0] > ((b->flag->f_length & F_L_DIEZE) + t[3] + t[2]))
		{
			add_in_buf(b, ' ');
			t[0]--;
		}
	...
	if (b->flag->f_precision > 0)
		...
		while (t[0]-- > t[3])
			add_in_buf(b, '0');
	else if (buf[0] != '0' && b->flag->f_length & F_L_DIEZE)
		add_in_buf(b, '0');
	t[3] = -1;
	while (buf[++t[3]])
		add_in_buf(b, buf[t[3]]);
...
static void			do_o_next(int *t, t_buf *b, char *buf, long double nb)
{
	t[3] = ft_itoa_base_buffer(nb, 8, buf);
	if (nb != 0 && b->flag->f_length & F_L_DIEZE &&
		b->flag->f_precision <= t[3])
	{
		b->flag->f_precision = t[3] + 1;
		t[0]++;
	}
	if (b->flag->f_length & F_L_MOINS)
		do_o_f_l_moins(t, buf, b);
	else
		do_o_wthout_f_l_moins(t, buf, b);
}
...
void				do_o(t_buf *b, va_list *pa, char type)
{
	...
	t[1] = 0;
	t[2] = 0;
	if (b->flag->f_length & F_L_DIEZE && b->flag->f_precision == 0)
		b->flag->f_precision = 1;
	...
	t[0] = (b->flag->f_precision > b->flag->f_lenmin) ? b->flag->f_precision :
		b->flag->f_lenmin;
	nb = chk_o_modifier(pa, b->flag, type);
