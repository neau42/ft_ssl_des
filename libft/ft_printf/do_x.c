...
/*   Updated: 2016/11/19 02:39:20 by nboulaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static long double	chk_x_modifier(va_list *pa, t_flags *flag)
{
	long double nb;
...
static void			do_x_f_l_moins(int *t, char *buf, t_buf *b, char type)
{
	if (t[2] && (t[0]-- || 1))
	{
		add_in_buf(b, '0');
...
static void			do_x_wthout_f_l_moins(int *t, char *buf, t_buf *b, char c)
{
...
static void			do_x_next(int *t, char *buf, t_buf *b, char type)
{
	if (type == 'X')
		ft_capitaliz(buf);
	if (b->flag->f_length & F_L_MOINS)
		do_x_f_l_moins(t, buf, b, type);
	else
		do_x_wthout_f_l_moins(t, buf, b, type);
}
