...
/*   Updated: 2018/12/04 15:33:34 by nboulaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_ssl.h"

static void			init_vars(const uint32_t **k, const uint32_t **s,
						uint32_t *abcdfgi, t_chksum *sum)
{
	...
	ft_memcpy(abcdfgi, sum->md5, sizeof(uint32_t) * 4);
	abcdfgi[6] = -1;
}

void				md5(const uint32_t *msg, t_chksum *sum)
{
	static const uint32_t	*s = NULL;
