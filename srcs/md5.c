...
/*   Updated: 2018/12/14 06:42:11 by nboulaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// static void			init_vars(const uint32_t **g_k1, const uint32_t **g_s1,
// 						uint32_t *abcdfgi, t_chksum *sum)
// {
// 	// if (!*g_s1)
// 	// {
// 	// 	*g_k1 = k1;
...
void				md5(const uint32_t *msg, t_chksum *sum, uint32_t opts)
{
	...
	(void)opts;
	ft_memcpy(abcd, sum->md5, sizeof(uint32_t) * 4);
	abcd[6] = -1;
	while (++abcd[6] < 64)
	{
		...
		abcd[4] = abcd[4] + abcd[0] + g_k1[abcd[6]] + msg[abcd[5]];
		abcd[0] = abcd[3];
		abcd[3] = abcd[2];
		abcd[2] = abcd[1];
		abcd[1] += (abcd[4] << g_s1[abcd[6]]) | (abcd[4] >> (32 - g_s1[abcd[6]]));
	}
	sum->md5[0] += abcd[0];
	sum->md5[1] += abcd[1];
	sum->md5[2] += abcd[2];
