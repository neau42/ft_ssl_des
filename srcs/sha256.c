...
/*   Updated: 2018/11/23 03:18:32 by nboulaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_chksum *r)
{
	...
	abcdfgi[0] = r->sha256[0];
	abcdfgi[1] = r->sha256[1];
	abcdfgi[2] = r->sha256[2];
	abcdfgi[3] = r->sha256[3];
	abcdfgi[6] = -1;
}
...
void				sha256(const uint32_t *msg, t_chksum *sum)
{
	...
	sum->sha256[0] = (sum->sha256[0] + abcd[0]);
	sum->sha256[1] = (sum->sha256[1] + abcd[1]);
	sum->sha256[2] = (sum->sha256[2] + abcd[2]);
	sum->sha256[3] = (sum->sha256[3] + abcd[3]);
}
